			...
			await html2pdf()
				...
				.then(async (pdf: any) => {
					...
					for (var i = 1; i <= totalPages; i++) {
						...
						pdf.text(
							`Preview at ${$page.url.protocol}//${$page.url.host}/preview/${(await invoice).$id}`,
						...
						// pdf.fromHTML(
						// 	`<div>
						// 		<p>Preview at <a href="${$page.url.protocol}//${$page.url.host}/preview/${(await invoice).$id}">${
						// 			$page.url.protocol
						// 		}//${$page.url.host}/preview/${(await invoice).$id}</a></p>
						// 		<p>Generated by <a href="${$page.url.protocol}//${$page.url.host}">${$page.url.host}</a></p>
						// 	</div>`,
						// 	pdf.internal.pageSize.getWidth() / 2,
...
{:then invoice}
	...
	<div class="flex flex-wrap items-center gap-4">
		...
		<Button on:click={copyToClipboard} variant="secondary">Copy Invoice Link</Button>
		<!--
			Email body
		...
		-->
		<Button
			href="mailto:{invoice.contact.email}?subject=Your invoice is ready&body=Hi {invoice.contact
				.name},%0D%0A%0D%0AYour invoice is ready. Click the link below to view it.%0D%0A%0D%0A{$page
